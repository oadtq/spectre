    ...
    class App: ObservableObject {
        ...
        private static func setTitle(
            ...
            case GHOSTTY_TARGET_SURFACE:
                ...
                // Intercept special ghostty command sequences sent via OSC set-title.
                // These are consumed silently and never shown as a window title.
                if title.hasPrefix("ghostty-cmd:") {
                    let cmd = String(title.dropFirst("ghostty-cmd:".count))
                    handleCommand(cmd, from: surfaceView)
                    return
                }
        ...
        /// Handle a ghostty-cmd: command intercepted from the set-title OSC sequence.
        private static func handleCommand(_ cmd: String, from surfaceView: Ghostty.SurfaceView) {
            switch cmd {
            case "create-tab-group":
                NotificationCenter.default.post(
                    ...
                    object: surfaceView)
            default:
                Ghostty.logger.warning("unknown ghostty-cmd: \(cmd)")
            }
        }
