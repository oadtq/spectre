...
class AppDelegate: NSObject,
    ...
    func applicationShouldTerminate(_ sender: NSApplication) -> NSApplication.TerminateReply {
        ...
        let alert = NSAlert()
        alert.messageText = "Quit Spectre?"
        alert.informativeText = "All terminal sessions will be terminated."
        alert.addButton(withTitle: "Close Spectre")
    ...
    func application(_ sender: NSApplication, openFile filename: String) -> Bool {
        ...
        if requiresConfirm {
            ...
            let alert = NSAlert()
            alert.messageText = "Allow Spectre to execute \"\(filename)\"?"
    ...
    @objc private func ghosttyNewTab(_ notification: Notification) {
        ...
        // In vertical-tabs mode, add a tab to the existing controller
        // instead of creating a new window.
        if controller.isVerticalTabMode {
            controller.addVerticalTab(withBaseConfig: config)
            return
        }
