...
pub fn init(
    ...
    // Product name is set in Xcode project (Ghostty or Spectre)
    const product_name = switch (config.optimize) {
        .Debug => "Spectre[DEBUG]",
        .ReleaseSafe,
        .ReleaseSmall,
        .ReleaseFast,
    ...
    const app_path = b.fmt("macos/build/{s}/{s}.app", .{ xc_config, product_name });
