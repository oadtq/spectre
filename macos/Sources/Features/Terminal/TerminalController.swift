...
class TerminalController: BaseTerminalController, TabGroupCloseCoordinator.Controller {
    override var windowNibName: NSNib.Name? {
        ...
        case "vertical-tabs": "TerminalVerticalTabs"
        case "tabs":
#if compiler(>=6.2)
    ...
    /// Whether this controller is in vertical-tabs mode.
    private(set) var isVerticalTabMode: Bool = false

    /// The vertical tab model, only used when isVerticalTabMode is true.
    private(set) var verticalTabModel: VerticalTabModel?

    /// Cancellable for observing vertical tab model changes.
    private var verticalTabCancellables: Set<AnyCancellable> = []

    /// Flag to track when we're restoring surface tree from the model (during tab switches).
    /// When true, surfaceTreeDidChange should not update the model to avoid overwriting
    ...
    override func surfaceTreeDidChange(from: SplitTree<Ghostty.SurfaceView>, to: SplitTree<Ghostty.SurfaceView>) {
        ...
        // Keep the vertical tab model in sync (only if not restoring from model)
        if isVerticalTabMode, let verticalTabModel,
           let selectedId = verticalTabModel.selectedTabId,
           !isRestoringSurfaceTreeFromModel {
            verticalTabModel.updateSurfaceTree(for: selectedId, tree: to)
        }

        // If our surface tree is now nil then we close our tab or window.
        if to.isEmpty {
            if isVerticalTabMode, let verticalTabModel,
               let selectedId = verticalTabModel.selectedTabId,
               verticalTabModel.tabs.count > 1 {
                // Close just this vertical tab
                closeVerticalTabImmediately(id: selectedId)
            } else {
                self.window?.close()
            }
        }
    ...
    override func closeSurface(
        ...
        // Vertical tabs mode: close the current vertical tab
        if isVerticalTabMode, let verticalTabModel,
           let selectedId = verticalTabModel.selectedTabId {
            closeVerticalTab(id: selectedId)
            return
        }
    ...
    override func windowDidLoad() {
        ...
        if isVerticalTabMode, let verticalTabModel {
            ...
            let sidebarHosting = SidebarHostingView(rootView:
                VerticalTabSidebarView(
                    model: verticalTabModel,
                    onNewTab: { [weak self] in
                        guard let self, let surface = self.focusedSurface?.surface else { return }
                        self.ghostty.newTab(surface: surface)
                    },
                    onNewGroup: { [weak self] in
                        self?.createTabGroup(nil)
                    },
                    onCloseTab: { [weak self] tabId in
                        self?.closeVerticalTab(id: tabId)
                    }
                )
            )

            // Use a plain container with Auto Layout instead of NSSplitView
            ...
            NSLayoutConstraint.activate([
                // Sidebar: fixed width, pinned to left
                sidebarHosting.leadingAnchor.constraint(equalTo: container.leadingAnchor),
                sidebarHosting.topAnchor.constraint(equalTo: container.topAnchor),
                sidebarHosting.bottomAnchor.constraint(equalTo: container.bottomAnchor),
                sidebarHosting.widthAnchor.constraint(equalToConstant: VerticalTabSidebarView.defaultWidth),

                // Terminal: fills remaining space
                terminalContainer.leadingAnchor.constraint(equalTo: sidebarHosting.trailingAnchor),
            ...
            // Observe tab selection changes to swap the displayed surface tree
            verticalTabModel.$selectedTabId
                .removeDuplicates()
                .sink { [weak self] newId in
                    self?.verticalTabSelectionDidChange(to: newId)
                }
                .store(in: &verticalTabCancellables)

            // Observe window title changes to update the current vertical tab title
            window.publisher(for: \.title)
                .sink { [weak self] newTitle in
                    guard let self, let verticalTabModel = self.verticalTabModel,
                          let selectedId = verticalTabModel.selectedTabId else { return }
                    verticalTabModel.updateTitle(for: selectedId, title: newTitle)
                }
                .store(in: &verticalTabCancellables)
        } else {
    ...
    @IBAction func createTabGroup(_ sender: Any?) {
        guard isVerticalTabMode, let verticalTabModel else { return }
        guard let selectedId = verticalTabModel.selectedTabId else { return }
        let basePath = focusedSurface?.pwd ?? verticalTabModel.selectedTab?.pwd
    ...
    override func focusedSurfaceDidChange(to: Ghostty.SurfaceView?) {
        ...
        focusedSurface.$backgroundColor
            .sink { [weak self, weak focusedSurface] _ in 
                ...
                guard let self, let verticalTabModel = self.verticalTabModel else { return }
                if let bgColor = focusedSurface?.backgroundColor {
                    verticalTabModel.updateTerminalBackgroundColor(NSColor(bgColor))
                } else if let focusedSurface {
                    // backgroundColor is nil until the surface renders; fall back to config color
                    verticalTabModel.updateTerminalBackgroundColor(NSColor(focusedSurface.derivedConfig.backgroundColor))
                } else {
                    verticalTabModel.updateTerminalBackgroundColor(nil)
                }
            }
            .store(in: &surfaceAppearanceCancellables)

        // Immediately sync the current background color (don't wait for a change event)
        if let verticalTabModel {
            if let bgColor = focusedSurface.backgroundColor {
                verticalTabModel.updateTerminalBackgroundColor(NSColor(bgColor))
            } else {
                verticalTabModel.updateTerminalBackgroundColor(NSColor(focusedSurface.derivedConfig.backgroundColor))
    ...
    override func pwdDidChange(to: URL?) {
        ...
        // Update the vertical tab model with the new path
        if isVerticalTabMode, let verticalTabModel = self.verticalTabModel,
           let selectedId = verticalTabModel.selectedTabId {
            verticalTabModel.updatePwd(for: selectedId, pwd: to?.path)
        }
    }
    ...
    @objc private func onMoveTab(notification: SwiftUI.Notification) {
        ...
        // Vertical tabs mode: use the model
        if isVerticalTabMode, let verticalTabModel {
            verticalTabModel.moveSelectedTab(by: action.amount)
            return
        }
    ...
    @objc private func onGotoTab(notification: SwiftUI.Notification) {
        ...
        if isVerticalTabMode, let verticalTabModel {
            if tabIndex == GHOSTTY_GOTO_TAB_PREVIOUS.rawValue {
                verticalTabModel.selectPreviousTab()
            } else if tabIndex == GHOSTTY_GOTO_TAB_NEXT.rawValue {
                verticalTabModel.selectNextTab()
            } else if tabIndex == GHOSTTY_GOTO_TAB_LAST.rawValue {
                if let last = verticalTabModel.tabs.last {
                    verticalTabModel.selectTab(id: last.id)
                }
            } else if tabIndex >= 1 {
                verticalTabModel.selectTab(at: Int(tabIndex))
            }
            return
        }
    ...
    @objc private func onCloseTab(notification: SwiftUI.Notification) {
        ...
        if isVerticalTabMode, let verticalTabModel {
            ...
            guard let tab = verticalTabModel.tabs.first(where: { $0.surfaceTree.contains(target) }) else { return }
            closeVerticalTab(id: tab.id)
            return
        }
    ...
    @objc private func onCreateTabGroup(notification: SwiftUI.Notification) {
        guard isVerticalTabMode, let verticalTabModel else { return }
        guard let target = notification.object as? Ghostty.SurfaceView else { return }
        guard surfaceTree.contains(target) else { return }
        guard let selectedId = verticalTabModel.selectedTabId else { return }

        // Use the current pwd as the group's base path
        let basePath = focusedSurface?.pwd ?? verticalTabModel.selectedTab?.pwd

        // Move the active tab into a new group rooted at the current directory
        verticalTabModel.moveTabToNewGroup(tabId: selectedId, basePath: basePath)
    }
    ...
    /// Whether a surface belongs to any tab in the vertical tab model.
    func verticalTabContainsSurface(_ surface: Ghostty.SurfaceView) -> Bool {
        guard let verticalTabModel else { return false }
        return verticalTabModel.tabs.contains { tab in
            tab.surfaceTree.contains(surface)
        }
    }

    /// Add a new vertical tab with a fresh surface.
    func addVerticalTab(withBaseConfig base: Ghostty.SurfaceConfiguration? = nil) {
        guard let verticalTabModel, let ghostty_app = ghostty.app else { return }

        // Save the current tab's state before switching
        saveCurrentVerticalTabState()

        // If the active tab is inside a group that has a base path, inherit it
        // so the new terminal starts in the same directory.
        var config = base
        if config?.workingDirectory == nil, let groupPath = verticalTabModel.activeGroupBasePath {
            var cfg = base ?? Ghostty.SurfaceConfiguration()
            cfg.workingDirectory = groupPath
            config = cfg
        }
        ...
        // Add the tab
        verticalTabModel.addTab(surfaceTree: newTree)

        // Switch to the new tab's surface tree
        isRestoringSurfaceTreeFromModel = true
        self.surfaceTree = newTree
        isRestoringSurfaceTreeFromModel = false
        self.focusedSurface = newSurface
        Ghostty.moveFocus(to: newSurface, from: nil)
    }

    /// Close a vertical tab by ID.
    func closeVerticalTab(id: UUID) {
        guard let verticalTabModel else { return }

        // If this is the last tab, close the window
        if verticalTabModel.tabs.count <= 1 {
            closeWindow(nil)
            return
        }

        // Check if any surface in this tab needs quit confirmation
        if let tab = verticalTabModel.tabs.first(where: { $0.id == id }),
           tab.surfaceTree.contains(where: { $0.needsConfirmQuit }) {
            confirmClose(
                messageText: "Close Tab?",
                informativeText: "The terminal still has a running process. If you close the tab the process will be killed."
            ) { [weak self] in
                self?.closeVerticalTabImmediately(id: id)
            }
            return
        }

        closeVerticalTabImmediately(id: id)
    }

    private func closeVerticalTabImmediately(id: UUID) {
        guard let verticalTabModel else { return }
        let wasSelected = verticalTabModel.selectedTabId == id

        // Note: we do NOT call ghostty.requestClose for each surface here.
        // Doing so fires a notification chain (ghosttyDidCloseSurface → removeSurfaceNode →
        // replaceSurfaceTree with empty tree → surfaceTreeDidChange) that re-enters
        // closeVerticalTabImmediately before this call returns, causing a double-free crash.
        // The surfaces are freed naturally when SurfaceView objects are deallocated
        // (SurfaceView deinit calls ghostty_surface_free via Ghostty.Surface).

        verticalTabModel.removeTab(id: id)

        // If the closed tab was selected, switch to the newly selected tab
        if wasSelected, let newTab = verticalTabModel.selectedTab {
            isRestoringSurfaceTreeFromModel = true
            self.surfaceTree = newTab.surfaceTree
            isRestoringSurfaceTreeFromModel = false
            if let focused = newTab.focusedSurface ?? newTab.surfaceTree.first {
                self.focusedSurface = focused
                Ghostty.moveFocus(to: focused, from: nil)
            }
        }
    }

    /// Called when the vertical tab selection changes.
    private func verticalTabSelectionDidChange(to newId: UUID?) {
        guard let verticalTabModel, let newId else { return }
        guard let tab = verticalTabModel.tabs.first(where: { $0.id == newId }) else { return }
        ...
        isRestoringSurfaceTreeFromModel = false
        if let focused = tab.focusedSurface ?? tab.surfaceTree.first {
            // Re-establish title+bell Combine subscriptions on the new surface.
            // Setting focusedSurface directly only calls syncFocusToSurfaceTree(),
            // it does NOT re-subscribe to $title/$bell for the window title.
            focusedSurfaceDidChange(to: focused)
            DispatchQueue.main.async {
                Ghostty.moveFocus(to: focused, from: nil)
            }
        }
    ...
    /// Save the current surface tree and focused surface back to the current vertical tab.
    private func saveCurrentVerticalTabState() {
        guard let verticalTabModel, let currentId = verticalTabModel.selectedTabId else { return }
        verticalTabModel.updateSurfaceTree(for: currentId, tree: surfaceTree)
        verticalTabModel.updateFocusedSurface(for: currentId, surface: focusedSurface)
    }
